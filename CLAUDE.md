...
| `js/palette-drag.js` | **Shared** pointer-based palette→canvas drag (`PaletteDrag.attach`). Replaces HTML5 drag-and-drop, which is dead on touch/pen. Used by both apps. |
...
**Touch / mobile input:** `js/touch.js` (`TouchBridge.attach(canvas, handlers)`) is a small pointer bridge shared by both apps — it feeds a synthetic mouse-like event `{clientX, clientY, button:0, shiftKey, preventDefault}` into the app's **existing** `onCanvasDown/Move/Up/Context` handlers, so desktop behaviour is untouched. It adds: **one-finger drag** = pan/move (whatever the mouse would do), **two-finger pinch** = zoom about the midpoint (mutates `view` in place via `getView()`), and **press-and-hold ~600ms** = the context menu (`context` handler). Canvases set `touch-action: none` and the viewport meta disables page zoom so gestures don't scroll the page. The digital app also gained **tap-to-place** (arm a palette part with a click/tap → `_tool`, then tap the sheet → `placeTool`; mirrors the analog app's `App.tool` model) and a **⇧ Select** toolbar box (`_shiftLock`, read via `shiftHeld(e)`) that latches Shift for touch so marquee-select / multi-select work without a keyboard. `TouchBridge` is loadable headlessly (pure IIFE, no DOM) — see the scratchpad touch test.

**Palette drag-and-drop:** `js/palette-drag.js` (`PaletteDrag.attach(cfg)`) drags a part out of a palette onto a canvas using **pointer** events, so mouse, finger and stylus all take one code path. It replaced HTML5 drag-and-drop (`draggable` + `dragstart`/`drop`), which works with a mouse but does nothing at all on touch/pen — that gap was why the palettes grew click-to-arm fallbacks. It's delegated to the palette container (tiles can be rebuilt freely), starts only after `THRESHOLD` = 6px of travel (so a plain click still falls through to each palette's own click handler and arms the tool), shows a `.pal-ghost` under the pointer that lights up over the drop target, closes the mobile drawers on drag start (`MobileDrawers.closeAll`, exported by `mobile.js`), and cancels on Escape or `pointercancel`. Tiles set `touch-action: pan-y` so a sideways pull drags the part while a vertical one still scrolls the list. Wired in `initPaletteDrag()` (ui.js → `dropPaletteItem` → `addAt`/`addChipAt`) and `Analog.initPaletteDrag()` (analog/ui.js → `Analog.addPartAt`).
...
**Boolean formula → circuit (engine.js — pure):** the inverse of the expression tracer. `parseBoolFormulas(text)` parses one formula per line/`;` (`Name = expr` names the OUT; unnamed get Q/Q2/…) — syntax: AND `· & && * .` or juxtaposition (`A B`, `A(B+C)`), OR `+ | ||`, XOR `^ ⊕`, NOT prefix `! ~ ¬` / postfix `'`, case-insensitive keywords `AND OR XOR NAND NOR XNOR NOT`, constants `0`/`1`, parens; precedence NOT > AND > XOR > OR. The tracer's own `exprToText` output re-parses. AST ops are n-ary and flattened (n-ary XOR = parity, matching `evalGate`); NAND/NOR/XNOR keywords apply pairwise left-associative; `notNode` collapses double negation. `synthBoolCircuit(text)` builds a live circuit: one `IN` per variable (first-appearance order), `HIGH`/`LOW` for constants, `not(op)` → a single NAND/NOR/XNOR gate, subexpressions deduped via sorted canonical keys (`boolAstKey`, so `A·B` ≡ `B·A` share one gate), runs wider than 8 inputs chunk into gate trees, and a layered column layout (`COL_PITCH` 144: INs left, gates by source depth each near the mean Y of its sources, OUTs right). UI: the **ƒ Formula** toolbar button (edit mode) opens `#formulaPanel` (`openFormulaPanel`/`buildFromFormula` in ui.js, Ctrl+Enter builds) — drops the block right of existing parts (else at the view origin), dedupes IN/OUT labels like paste, and selects the new comps. The smoke test proves each synthesized circuit matches `evalBoolAst` on every input combination.
...
**Model:** components have **terminals** (not typed pins) at rotated logical offsets (`Analog.terminalPos`). Wires join two terminals `{c, t}`. `Analog.TYPES` is the catalogue — `RES`, `POT` potentiometer (terminals `[endA, wiper, endB]`, `c.value` = total Ω + `c.ratio` 0–1 wiper position, stamped as two series resistors), `CAP`, `IND`, `LAMP` (a resistor whose glow tracks power against `watts`), `FUSE` (`value` = amp rating; `c._blown` opens it when exceeded), `DCV`, `ACV` (AC source, has a `freq`), `SQV` square source (±`value` at `freq`, 50 % duty), `ISRC` ideal DC current source, `GND`, `VM` voltmeter, `AM` ammeter, `SCOPE` oscilloscope, plus the **nonlinear** semiconductors `DIODE`, `ZENER` (breaks down near nameplate `c.value` volts), `LED`, `NPN`, `PNP`, plus **switches/relays** `SW`, `PUSH`, `RELAY` — each with `value`/`unit`; C/L/AC/SQV carry `reactive:true` (→ `Analog.isTransient`), `SCOPE` carries `scope:true`, and the semiconductors carry `nonlinear:true` (→ `Analog.isNonlinear`) with device params (`is`/`n` for diodes; `is`/`bf`/`br`/`npn` for BJTs — a BJT's `c.value` is its editable β/`bf`). Diode/LED terminals are `[anode, cathode]`; BJT terminals are `[collector, base, emitter]`. `SW`/`PUSH` carry `switchable:true` (→ `Analog.isSwitch`) and a manual `c.closed` bool (`PUSH` is `momentary`); `RELAY` (terminals `[coil+, coil−, contactA, contactB]`, `c.value` = coil Ω) has an auto `c._on` contact state. There is **no wire-value**; instead electrical **nodes** are derived: `buildNodes()` runs union-find over wire-connected terminals; every set is a node; terminals on a `GND` collapse to the datum (`"gnd"`, 0 V).

**Serialization (model.js, pure):** `serializeCircuit(circ, only?)` → plain data (user fields only — `rot/value/freq/closed/ratio/label`; runtime `_*` state is dropped; with `only` it's a subset for copy/paste, keeping wires whose both ends are inside). `instantiateData(data, dx, dy)` materialises it with **fresh ids** (safe to paste into any sheet; unknown types/dangling wires dropped); `deserializeCircuit(data)` rebuilds a whole circuit. Save/load uses localStorage key `logiclab.analog.v1` (auto-restored on init).

**Engine — Modified Nodal Analysis.** `_anBuild(circ, mode, dt, time, gv, nlState)` stamps `A·x = z` (`x` = [non-datum node voltages …, voltage-source branch currents …]); `_anSolveMode` solves it by Gaussian elimination (`_anSolve`) and wraps the result (`volt`/`current`/`termCurrent`/`meter`). Two modes (dc/tran) share the builder.
...
**Switches, relays & fuses — linear, stateful.** A switch/push-button stamps a plain conductance that flips between `_SW_RON` (≈0, closed) and `_SW_ROFF` (≈∞, open) from `c.closed`; clicking one in sim toggles it (`PUSH` is momentary — held closed only while pressed, released in `_anUp` via `App.pushHeld`). A `FUSE` stamps `_SW_RON` until its current exceeds `c.value` amps, then `c._blown = true` opens it (one-way per run; "🔧 Replace fuse" in the sim context menu or a new run resets it). A `RELAY` stamps its coil as a resistor (`c.value` Ω, terminals 0/1) and its normally-open contact (terminals 2/3) as an on/off conductance driven by `c._on`. `_anSolveMode` recomputes `c._on` each solve from the coil current with hysteresis (pull-in at `TYPES.RELAY.pull`, drop-out at half). Because contact/fuse state changes the network, **`solveDC` re-solves in an outer loop until every relay's `_on` and fuse's `_blown` settles** (≤20 passes); in transient the state evolves one step at a time. `initTransient` de-energises all relays and replaces all fuses so each run starts clean.

The two linear analysis modes:
- **DC (`solveDC`):** resistive, linear → one exact solve. Capacitor = open (skipped); inductor = short (0 V source). Stamps: resistor/lamp = `1/R`; pot = two resistors split by `ratio` (each half clamped ≥ `RES.min`); current source = `inject()` into its nodes; DC/AC/square source & ammeter & DC-inductor = branch unknown + `V(+)−V(−)=E`; voltmeter/scope = ideal open (probed, not stamped); ground = datum.
- **Transient (`stepTransient(circ, dt, time)`):** backward-Euler **companion models** — capacitor = conductance `C/dt` ∥ current source holding `c._vc`; inductor = conductance `dt/L` ∥ current source holding `c._il`; AC source `E = amp·sin(2π·f·time)`; square source `E = ±amp` by half-period (`_sqPhase`). Each step solves, reads currents from the OLD state, then advances `c._vc`/`c._il`. `initTransient()` zeroes that state; `characteristicTime()` estimates the slowest RC/RL/AC timescale to auto-pick `dt` and the scope window. A missing ground / floating section returns `{ ok:false, error }`.

**Zener** shares the diode NR branch: forward Shockley plus a reverse exponential `−Is·exp((−vd−Vz′)/vt)` with `Vz′ = c.value − 0.7` so the knee lands at the nameplate voltage (`_zenerVz`); junction limiting applies to whichever side is active (forward `vd`, or reverse `u = −vd−Vz′` kept in `st.u`).

**Sim loop:** on entering sim, `Analog.enterSim()` picks the mode. A purely resistive/DC circuit is solved **once** (`resolve()`). A circuit with any reactive part **time-steps**: an rAF loop (`Analog._frame`) advances `stepsPerFrame` steps per frame, records every scope's trace, and redraws — with a **Run/Pause** button (`#anRunBtn`) and a `t = …` readout (`#anTime`). Right-click a part → Change value / Wiper position (POT) / Open-Close (switches) / Replace fuse / Rename / Duplicate / Rotate / Delete (value edits apply next step; topology edits `afterStruct()` → restart the run). In sim, **click a meter** → a draggable readout window; a `SCOPE` opens an oscilloscope window that live-plots its recorded trace (`_anDrawScope`); **hover anything** → a probe tooltip with live V/I/P (node voltage on terminals & wires; `App.probe`, drawn by `_drawProbe`); **drag a POT** horizontally to sweep its wiper live. `Analog.fmt` renders SI units (`1.5 kΩ`, `5 mA`, `20 µs`).

**Placing parts (analog):** three routes, all funnelling into `Analog.addPartAt(type, wx, wy)` (snaps, auto-labels, selects, snapshots) — **drag** a tile out of the palette (`Analog.initPaletteDrag`, see the palette drag-and-drop note above), **click** a tile to arm `App.tool` then click the sheet (`_anDown`), or **right-click empty sheet** → `Analog.showAddMenu(wx, wy, sx, sy)`, which renders the whole `AN_PALETTE` as submenus and drops the pick where you clicked. `_anShowMenu`/`_anMenuLevel` build nested levels (`submenu`/`sep`/`disabled`/`danger`) using the **digital tab's `#ctxMenu` class names and stylesheet block** — the CSS selectors are shared, `#anCtxMenu` only overrides `position: fixed` (it positions in viewport coords).

**Current-flow animation (analog):** in sim mode, dots ride each wire in the direction of conventional current, faster where more of it flows. Toggled by the **◦◦ Flow** toolbar button (`#anFlowBtn`, `App.flow`, on by default).

Two pieces, split along the usual purity line:
- **`Analog.wireCurrents(circ, res)` (engine.js — pure, tested).** MNA solves for *node* voltages, which say nothing about how a node's current divides among the individual wires that form it. But those wires are a graph whose vertices (terminals) have known injections, so on a **tree** the flow is forced: each wire carries the net injection of everything hanging off its far side (BFS spanning tree, then accumulate leaves-first). Wires closing a loop *inside one node* stay at 0 — the split between ideal parallel conductors is genuinely undefined. It works over **wire-connected clusters, not electrical nodes** (two `GND` symbols are one node yet aren't wired together), and a `GND` terminal is its cluster's **slack**, absorbing the leftover because the datum carries its return. Feeds on `res.termCurrent(c, t)` = current *out of* one terminal (built in `_anSolveMode` alongside `cur`, using the same pre-update relay/fuse state): two-terminal parts just split their element current — sign flipped for sources, which deliver *out of* terminal 0 — while POT/RELAY/BJT get their own internal topology. Per component they sum to zero (KCL).
- **`_drawFlow` (render.js).** `w._flow` is a phase kept as a **fraction of one dot spacing**, so it survives re-routing and moved parts. Spacing is the wire's length over a whole number of dots, so one dot leaves the far end exactly as another enters — no flicker on short wires. Speed is `AN_FLOW_SPEED · (|I|/ref)^0.55`, i.e. **relative to the busiest wire**, which is what makes a µA circuit as readable as an amp one (a 100× current range → ~12.6× speed range; nothing freezes, ordering stays obvious). `App.flowRef` **decays** toward the present (`AN_FLOW_DECAY`) rather than tracking the instantaneous peak — otherwise an AC circuit would run flat out even at its zero crossing instead of visibly slowing and reversing. Below `AN_FLOW_MIN` of the reference, no dots at all (so an open switch's 1e9 Ω leakage reads as still). Drawn over the wires but under the parts.

Frame driving: a **transient** run already redraws every step, so the dots ride along for free and freeze on pause. A **static DC** solve draws once, so `Analog.syncFlowLoop()` (called from `resolve()`, `enterSim`/`exitSim`, the tab switch and the toggle) runs an rAF loop — only while there's something to animate, and it bails when the digital tab is in front. `Analog.render()` derives the animation's `dt` from **wall-clock** time, so extra renders (hover, pan) can't speed the dots up. Hovering a wire also reports its current in the probe tooltip.

**Sheet background (analog):** matched to the digital tab — `AN_BG` `#151a21` / `AN_GRID` `#222b36` (= `COL.bg`/`COL.grid`), and the grid dots are drawn in **screen space** before the view transform (2px dots every `GRID × scale`, skipped below a 9px step) so they stay crisp at any zoom instead of scaling to sub-pixel. `#anStage` carries the same background so there's no flash on tab switch.

**Editing UX (analog):** parts get auto **designators** on placement (`R1`, `C2`, `Q1`… via `Analog.autoLabel`/`AN_PREFIX`, drawn above the part; Rename… to change). **Save/Load** buttons persist the sheet to localStorage (auto-restored on init). **Undo/redo** (`Analog.hist`, toolbar buttons + `Ctrl+Z`/`Ctrl+Y`, edit mode only) snapshots the serialized sheet after every mutation — call `Analog.snapshot()` after any structural/value edit. **Copy/paste/duplicate** (`Ctrl+C/X/V`, context menu) go through `serializeCircuit(circ, selection)` → `instantiateData` (fresh ids; auto-pattern labels re-designated; route scalars offset per axis by parity). **Shift+drag** on empty space box-selects (`App.drag.marquee`). `R` rotates the selection. The canvas is **HiDPI-aware** (`App.dpr` scales the backing store in `Analog.resize`/`render`; scope canvases too). The digital app's `onKeyDown` ignores keys while the analog tab is in front (and `_anKey` vice-versa).

**Wire routing (analog, model.js — pure):** wires are **orthogonal polylines**, digital-style but generalised for terminals that face any direction. Shape = `w.h0` (first segment horizontal?) + `w.route`, scalars that **alternate axes** (h0 → X, Y, X, …), each the endpoint of a segment in the current direction; after the route the path **closes onto terminal B** with ≤ 2 segments (continue to align, then turn in). No stored route → `defaultRoute` (leave the source terminal along its lead via `terminalDir`; straight if aligned, else a Z through the snapped midpoint) recomputed every render, so auto wires follow moved parts. `wireSegs(circ, w)` returns every segment (degenerates included, indices stable) tagged `routeIdx` = which scalar sets its lateral position (−1/−2 = pinned to terminal A/B); `wirePath` is the deduped point list used by **both** drawing and hit testing (`hitWireSeg`, point-to-segment). **Drawing:** mousedown on a terminal arms `App.wiring` (`{h0, route, x, y}`); release on empty space **plants a bend** at the snapped preview corner and keeps going (each click turns the corner; the live preview shows the leg + a dashed turn); release on another terminal finishes (bends stored on the wire); release on the start terminal keeps it armed (click-click routing); Esc/right-click cancels. **Dragging:** mousedown on a segment calls `grabWireSeg` — interior segments map straight to their scalar; terminal-pinned end segments **materialise extra bends** (unshift/push a pair, parity preserved) so they become draggable; move writes the snapped lateral coordinate into `route[idx]`. Right-click a wire → **Straighten** (drops `route`/`h0` back to auto) / Delete wire. Routes round-trip serialization.

**Roadmap:** all planned analog phases are **done** — DC foundation, transient engine (C/L + oscilloscope + AC), nonlinear semiconductors (diodes/zeners/LEDs/BJTs via Newton-Raphson), switches/relays/fuses, sources (square/current), pot/lamp, the current-flow animation, and the editing UX above (save/undo/copy-paste/probe). Possible next steps: AC frequency-domain sweep (Bode plots via complex MNA), transformers/coupled inductors, MOSFETs. (Parked elsewhere: the digital gate-built RAM 16×8 — see `build-a-computer-roadmap`.)
