...
**Roadmap:** all planned analog phases are **done** — DC foundation, transient engine (C/L + oscilloscope + AC), nonlinear semiconductors (diodes/zeners/LEDs/BJTs via Newton-Raphson), switches/relays/fuses, sources (square/current), pot/lamp, and the editing UX above (save/undo/copy-paste/probe). Possible next steps: AC frequency-domain sweep (Bode plots via complex MNA), transformers/coupled inductors, MOSFETs. (Parked elsewhere: the digital gate-built RAM 16×8 — see `build-a-computer-roadmap`.)
